...
if [ -x "$(command -v podman)" ]; then
    if [ "$(uname -s)" = "Linux" ]; then
        ...
        --env BRIDGE_PLUGIN_PROXY='{"services": [{"consoleAPIPath": "/api/proxy/plugin/distributed-tracing-plugin/backend/", "endpoint":"https://play.grafana.org/api/datasources/proxy/uid/grafanacloud-traces","authorize":true}]}' \
        $CONSOLE_IMAGE
    fi
else
